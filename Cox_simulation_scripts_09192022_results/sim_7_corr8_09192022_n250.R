...
for(case.id in 1:sim.total.n){
  ...
  cov.value <- 0.8 ## the same covariance for all features
  ...
  #  head(data)
  data$OS.status <- ifelse(runif(n)>0.9,  0,1)   ## 50% right-censored ## 1   ## no censored data ## 
  ...
  tryCatch({## ------------- fitting multivariate gaussian.regression with features selected by Gaussian model with 2 covariates:
  ...
  sim.summary$multivariate.gaussian.regression.p.values[case.id] <- paste(paste(row.names(fit.results),
                                                                                ": ",
                                                                                fit.results[,4],
                                                                                sep=""), collapse=";  " )}, error = function(e){})
  
  tryCatch({## ------------------------ fit lasso regularized Cox:
  ...
  sim.summary$glmnet.Cox.lambda.1se.beta[case.id] <- paste(paste(rownames(tmp.fit.2$beta)[1:5],
                                                                 ...
                                                                 tmp.fit.2$beta[1:5],
                                                                 sep=""), collapse=";  " )
  sim.summary$glmnet.Cox.lambda.1se.FDR[case.id] <- length(which(tmp.fit.2$beta[6:10]!=0))/5}, error = function(e){})
  
  tryCatch({## ------------------------ fit Gaussian model with elastic net regularization:
  ...
  sim.summary$glmnet.Gaussian.FDR[case.id] <- length(which(tmp.beta[6:10]!=0))/5}, error = function(e){})
}

save(sim.summary, file="Cox_regression_simulation_n250_sim_7_corr8_09192022summary.RData")
...
## --------------
sink("sim_7_corr8_09192022_n250_results_summary.txt")
head(sim.summary)
cat("\n");cat("----------------------- Sensitivity ---------------------------------\n");
