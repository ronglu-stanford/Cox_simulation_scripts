...
for(case.id in 1:sim.total.n){
  ...
  #  head(data)
  data$OS.status <- ifelse(runif(n)>0.5,  0,1)   ## 50% right-censored ## 1   ## no censored data ## 
...
save(sim.summary, file="Cox_regression_simulation_n250_sim_8_corr8_09192022summary.RData")
...
## --------------
sink("sim_8_corr8_09192022_n250_results_summary.txt")
head(sim.summary)
cat("\n");cat("----------------------- Sensitivity ---------------------------------\n");
